...
class ZebraPrinterService:
    ...
    def print_zpl(self, zpl_content: str, printer_name: str, job_name: str = "ZPL Lote") -> None:
        if not zpl_content.strip():
            raise PrinterError("Conteudo ZPL vazio.")

        if self.dev_mode or not _WIN32_DISPONIVEL:
            self._gravar_dev(zpl_content, printer_name, job_name)
            return
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    win32print.WritePrinter(handle, zpl_content.encode(self.encoding, errors="replace"))
                    win32print.EndPagePrinter(handle)
                finally:
                    win32print.EndDocPrinter(handle)
                log.info("Job %s enviado para %s (%d bytes)", job_id, printer_name, len(zpl_content))
            finally:
    ...
    def _gravar_dev(self, zpl_content: str, printer_name: str, job_name: str) -> None:
        ...
        arquivo.write_text(zpl_content, encoding=self.encoding)
        log.info(
